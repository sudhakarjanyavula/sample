...
RSpec.describe Api::UserController do
	let(:params) {{ :user => { :firstName => "sudhakar", :lastName => "janyavula",
	 ...
	 describe 'PATCH #update' do
	 	...
	 	let(:params) {{ id: user.id, user: { firstName: 'firstName', 
	...
	describe 'GET #typeahead' do
		...
		context 'is expected to match user with input' do
			let(:user1) {FactoryBot.create(:user, {:firstName => "sudhakar", :lastName => "janyavula",
				:email => "sudhakar@gmail.com"})}
			let(:user2) {FactoryBot.create(:user, {:firstName => "dhanvik", :lastName => "janyavula",
				:email => "dhanvik@gmail.com"})}
			context 'when user is provided' do
				it 'is expected to match user with input' do
					FactoryBot.create(:user, {:firstName => "sudhakar", :lastName => "janyavula",
					:email => "sudhakar@gmail.com"})
					FactoryBot.create(:user, {:firstName => "sudhakar", :lastName => "janyavula",
					:email => "sudhakar@gmail.com"})
